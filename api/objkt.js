export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok)
      return res.status(502).json({ error: "Objkt API nicht erreichbar. Bitte später erneut versuchen." });

    const data = await response.json();
    if (!data || !data.data)
      return res.status(404).json({ error: "Keine Daten von Objkt erhalten. Eventuell falsche Adresse." });

    const tokens = Object.values(data.data)[0];
    if (!tokens || tokens.length === 0)
      return res.status(404).json({ error: "Keine Werke gefunden. Eventuell reiner Collector oder Collab." });
