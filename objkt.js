export default async function handler(req, res) {
  ...
  const query = address.startsWith("KT") ? 
    `query($address:String!){ token(where:{fa_contract:{_eq:$address}},limit:15){ name description metadata tags } }` :
    `query($address:String!){ token(where:{_or:[{creator_id:{_eq:$address}},{issuer:{_eq:$address}},{minter:{_eq:$address}}]},limit:15){ name description metadata tags } }`;
